...
@section('content')
    ...
    <form method="POST" action="{{ route('evaluations.programme.update', $evaluation->id_evaluation) }}">
        @csrf
        @method('PUT')

        <div class="card theme-card shadow-sm">
            <div class="card-header theme-header">
                <h5 class="mb-0 fw-bold">Modifier la fiche de l'évaluation</h5>
            </div>
            <div class="card-body">
                <div class="row g-3">
                    <div class="col-md-3">
                        <label class="form-label">Date</label>
                        <input type="date" name="date_evaluation" class="form-control" value="{{ old('date_evaluation', $evaluation->date_evaluation) }}" required>
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">Libellé</label>
                        <input type="text" name="libeller" class="form-control" value="{{ old('libeller', $evaluation->libeller) }}" required>
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">Heure début</label>
                        <input type="time" name="heure_debut" class="form-control" value="{{ old('heure_debut', $evaluation->heure_debut) }}" required>
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">Heure fin</label>
                        <input type="time" name="heure_fin" class="form-control" value="{{ old('heure_fin', $evaluation->heure_fin) }}" required>
                    </div>
                    <div class="col-md-3">
                        ...
                        <select name="id_classe" id="id_classe" class="form-select" required>
                            ...
                            @foreach($classes as $classe)
                                <option value="{{ $classe->id_classe }}" data-ordre="{{ $classe->ordreEnseignement }}" @selected(old('id_classe', $first->id_classe) == $classe->id_classe)>{{ $classe->nom_classe }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">Matière</label>
                        <select name="id_matiere" id="id_matiere" class="form-select" data-selected="{{ old('id_matiere', $first->id_matiere) }}" required>
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="id_annee_scolaire" class="form-select" required>
                            ...
                            @foreach($annees as $annee)
                                <option value="{{ $annee->id_anneeScolaire }}" @selected(old('id_annee_scolaire', $first->id_annee_scolaire) == $annee->id_anneeScolaire)>{{ $annee->annee }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-3">
                        ...
                        <select name="id_note" class="form-select" required>
                            ...
                            @foreach($notes as $note)
                                <option value="{{ $note->id_note }}" @selected(old('id_note', $first->id_note) == $note->id_note)>{{ $note->typeNote }} / {{ number_format($note->valeur, 0, ',', ' ') }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-3 trimestre-field">
                        ...
                        <select name="id_trimestre" class="form-select">
                            ...
                            @foreach($trimestres as $trimestre)
                                <option value="{{ $trimestre->id_trimestre }}" @selected(old('id_trimestre', $first->id_trimestre) == $trimestre->id_trimestre)>{{ $trimestre->nom_trimestre }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-3 mois-field">
                        ...
                        <select name="mois" class="form-select">
                            ...
                            @foreach($moisOptions as $value => $label)
                                <option value="{{ $value }}" @selected(old('mois', $first->mois) == $value)>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
            ...
            <div class="card-footer bg-transparent">
                <div class="d-flex justify-content-between">
                    ...
                    <button type="submit" class="btn btn-primary px-4">Enregistrer la fiche</button>
                </div>
            </div>
        </div>
    </form>
