            ...
            $appels = AppelEpreuve::withoutGlobalScope('school')
                ...
                ->when($user->droit !== 'SupAdmin', function ($query) use ($user, $schoolId) {
                    ...
                    if ($user->droit === 'DAE' && $user->id_academie) {
                        $query->whereIn('controle_eleve.id_ecole', Ecole::withoutGlobalScopes()->where('id_academie', $user->id_academie)->pluck('idEcole'));
                        return;
                    }
                    if ($user->droit === 'DCAP' && $user->id_cap) {
                        $query->whereIn('controle_eleve.id_ecole', Ecole::withoutGlobalScopes()->where('id_cap', $user->id_cap)->pluck('idEcole'));
                        return;
                    }
                    $query->whereRaw('1 = 0');
                })
